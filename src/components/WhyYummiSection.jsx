  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto text-center">
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-poppins-extrabold text-[var(--color-brand-green)] mb-12 sm:mb-14">
          Mengapa Harus Yummi?
        </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8">
          {features.map((item) => (
            <div
              key={item.id}
              className="bg-white rounded-xl border-b-4 border-[var(--color-caramel)] shadow-md p-6 hover:shadow-xl hover:scale-[1.03] transition-transform duration-300"
            >
              <div className="w-12 h-12 sm:w-14 sm:h-14 mx-auto mb-4 flex items-center justify-center rounded-full bg-gradient-to-br from-[var(--color-green-dark)] to-emerald-400 text-white text-2xl sm:text-3xl shadow-md">
                {item.icon}
              </div>
              <h3 className="text-base sm:text-lg font-poppins-extrabold text-[var(--color-brown)] mb-2">
                {item.title}
              </h3>
